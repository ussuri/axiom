...
gdrivefsUtil.entryToStat_ = function(entry) {
  ...
  if (entry.mimeType === gdrivefsUtil.DIR_MIME_TYPE_) {
    ...
    var mimeTypes = entry.exportLinks ?
        Object.keys(entry.exportLinks).join('; ') : entry.mimeType;
    return new StatResult({
      dataType: 'blob',
      mimeType: mimeTypes,
      mode: mode,
...
gdrivefsUtil.downloadFile_ = function(entry, opt_mimeType) {
  if (entry.downloadUrl) {
    ...
    var url;
    if (opt_mimeType) {
      url = entry.exportLinks[opt_mimeType];
      if (!url) {
        return Promise.reject(new AxiomError.Incompatible(
      ...
      url =
          entry.exportLinks['text/plain'] ||
          entry.exportLinks['text/csv'];
      if (!url) {
        return Promise.reject(new AxiomError.NotFound(
            'text conversion', entry.title));
      }
    }
    return gdrivefsUtil.downloadUrl_(url);
  } else {
